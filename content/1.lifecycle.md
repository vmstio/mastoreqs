---
title: Support Lifecycle
description: We track dependencies for the five most recent Mastodon branches to help administrators determine upgrade path and deployment options. Administrators should strive to run supported releases on supported platforms to give their users the best, and most secure, experience possible.
navigation:
  icon: i-material-symbols-cycle
---

![Mastodon Branches](/mastodon-branches.png)

::tip
Any dependencies listed for development versions (nightly/alpha) of Mastodon are based on tracking the development process of the `main` branch on GitHub.
These and are subject to change at any time as project requirements change.
::

| Mastodon | Status                  | End of Life |                                                     |
| -------- | ----------------------- | ----------- | :-------------------------------------------------: |
| 4.6      | Main Development Branch |             | :icon{name="i-fluent-emoji-flat-check-mark-button"} |
| 4.5      | Latest Release Branch   |             |    :icon{name="i-fluent-emoji-flat-check-mark"}     |
| 4.4      | Previous Release Branch |             |    :icon{name="i-fluent-emoji-flat-check-mark"}     |
| 4.3      | End of Life             | 2026-05-06  |    :icon{name="i-fluent-emoji-flat-cross-mark"}     |
| 4.2      | End of Life             | 2026-01-08  |    :icon{name="i-fluent-emoji-flat-cross-mark"}     |
| 4.1      | End of Life             | 2025-04-08  |    :icon{name="i-fluent-emoji-flat-cross-mark"}     |
| 4.0      | End of Life             | 2023-10-31  |    :icon{name="i-fluent-emoji-flat-cross-mark"}     |
| 3.x      | End of Life             | 2023-12-31  |    :icon{name="i-fluent-emoji-flat-cross-mark"}     |

::collapsible

| Key                                          |                                        |
| -------------------------------------------- | -------------------------------------- |
| :icon{name="i-fluent-emoji-flat-check-mark"} | Supported release of Mastodon          |
| :icon{name="i-fluent-emoji-flat-warning"}    | Deprecated, use with caution           |
| :icon{name="i-fluent-emoji-flat-cross-mark"} | End of life, upgrade to latest release |

::

References: [Mastodon Security Policy](https://github.com/mastodon/mastodon/security)

::warning
For the purposes of this document, when applied to dependencies the term "End of Life" is standardized to mean when there are no further updates of any kind, and/or no support available for a released version of Mastodon or it's dependencies.
::

::caution
End of Life versions of Mastodon do not receive any features, bug fixes, and generally no security updates except at the discretion of the development team.
::
