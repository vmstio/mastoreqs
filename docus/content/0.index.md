...
The Mastodon instructions for installing from source use [rbenv](https://github.com/rbenv/rbenv) to install and manage the target Ruby version.
...
::alert{type="warning"}
Use of odd-numbered Node.js releases is not supported due to their short release lifespans and lack of testing with Mastodon dependencies.
::
...
Elasticsearch 7.x is the only officially supported version at this time due to limitations of the Chewy gem for Ruby.
Many administrators opt to use Elasticsearch 8.x and do so without issues, but if you're able to keep current with patching on 7.x at this time, you should.
...
Many administrators opt to use OpenSearch for this reason, without issues.
...
`vips -v` will show your currently running libvips version.
