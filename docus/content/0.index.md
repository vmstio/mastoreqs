...
::alert{type="warning"}
Elasticsearch 8.x (which still has the Mastodon necessary 7.x APIs) or OpenSearch (an API-compatible Elasticsearch 7.x fork) should be used with caution and are recommended for use only by experienced administrators who are willing/able to provide feedback.
::
